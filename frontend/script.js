...
async function loadRanking() {
  if (typeof window.ethereum !== "undefined") {
      try {
          ...
          const rankingData = parseRankingData(rankingString);
...
async function updateButtonState() {
  ...
  if (switchInput.checked) {
    ...
    try {
      ...
      alert("Não foi possível chamar a função");
      console.log(error);     
    }
  } else {
    voteButton.classList.add("button-off");
...
issueTokenButton.addEventListener("click", () => {
    ...
    alert(`Codename: ${codename}, Amount: ${amount}`);
});
